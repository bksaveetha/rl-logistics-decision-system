...
def _kpis(assignments: List[Dict[str, Any]], total_shipments: int) -> Dict[str, float]:
    if not assignments:
        return {"on_time_rate": 0.0, "no_show_rate": 1.0, "avg_cost": 500.0, "coverage": 0.0}
    n = len(assignments)
    return {
...
def grade_easy(episode_info: Dict[str, Any]) -> float:
    ...
    return max(0.0, min(1.0, 0.8 * k["on_time_rate"] + 0.2 * (1 - k["no_show_rate"])))

def grade_medium(episode_info: Dict[str, Any]) -> float:
    k = _kpis(episode_info["assignments"], episode_info["total_shipments"])
    cost_score = max(0.0, min(1.0, 1 - k["avg_cost"] / 600.0))
    return max(0.0, min(1.0, 0.5 * k["on_time_rate"] + 0.2 * (1 - k["no_show_rate"]) + 0.3 * cost_score))

def grade_hard(episode_info: Dict[str, Any]) -> float:
    k = _kpis(episode_info["assignments"], episode_info["total_shipments"])
    cost_score = max(0.0, min(1.0, 1 - k["avg_cost"] / 600.0))
    return max(0.0, min(1.0, 0.35 * k["on_time_rate"] + 0.2 * (1 - k["no_show_rate"]) + 0.25 * k["coverage"] + 0.2 * cost_score))